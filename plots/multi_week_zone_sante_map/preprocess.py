...
def preprocess_multi_week_zone_sante_map_data(data, parameters):
    """
    ...
    plot_data = plot_data.groupby(['year', 'epiweek', 'zone_sante']).size().reset_index(name='count')
    ...
    zone_sante_cases = plot_data.groupby(['zone_sante', 'date']).size().reset_index(name='count')
