...
def preprocess_multi_province_pyramid_data(data, parameters):
    """
    ...
    plot_data = data[[age_col, sex_col, 'province']].copy()
    ...
    plot_data = plot_data.groupby(['age_group', sex_col, 'province']).size().reset_index(name='count')

    # make the male counts negative for pyramid plot
    plot_data.loc[plot_data['sex'] == 'male', 'count'] *= -1

    # create a complete set of combinations of 'age_group', 'sex_col', and 'province'
    all_combinations = pd.MultiIndex.from_product(
        [plot_data['age_group'].unique(), plot_data[sex_col].unique(), plot_data['province'].unique()],
        names=['age_group', sex_col, 'province']
    )

    # reindex the grouped data to include all combinations and fill missing values with 0
    plot_data_complete = plot_data.set_index(['age_group', sex_col, 'province']).reindex(all_combinations, fill_value=0).reset_index()

    return plot_data_complete