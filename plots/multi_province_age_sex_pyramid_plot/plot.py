...
def plot_multi_province_pyramid(plot_data, parameters):
    ...
    num_rows = (num_provinces + num_cols - 1) // num_cols  # calculate the number of rows needed
    ...
    for i, province in enumerate(provinces):
        ...
        province_pivot['age_group_labels'] = province_pivot['age_group'].apply(get_nice_age_label)
        ...
        num_divs = 4
        max_population = max(province_pivot['male'].abs().max(), province_pivot['female'].abs().max())
    ...
    if export:
        ...
        # export to PDF
        fig.write_image(pdf_filename, format='pdf', width=fig_width, height=fig_height * num_rows)
