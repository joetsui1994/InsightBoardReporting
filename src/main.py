...
def create_report(args):
    print(
        "Creating an HTML report from the linelist file, this should just take a few seconds..."
    )
    ...
    report_html = generate_report_html(
        filtered_linelist, reporting_config, args.in_dir, args.out_dir
    )
...
def populate_variables(args):
    print(
        "Finding and replacing variables in the HTML file, this should just take a few seconds..."
    )
    ...
    while output_file.exists():
        output_file = (
            Path(args.out_dir)
            / f"{Path(args.in_file).stem}.populated.{filename_counter}.html"
        )
        filename_counter += 1
    with open(output_file, "w", encoding="utf-8") as f:
    ...
    print(
        f"{replacements_count} variables replaced and HTML file saved to '{output_file}'. Exiting..."
    )
...
def list_variables(args):
    ...
    VARIABLES = {
        variable: VARIABLE["description"] for variable, VARIABLE in VARIABLES.items()
    }
...
def cli(raw_args=None):
    parser = argparse.ArgumentParser(
        description="Command line tool for the report generation given a linelist file in .csv format (output from InsightBoard)."
    )
    ...
    parser_create = subparsers.add_parser(
        "create",
        help="Create a report from a linelist file according to an HTML template.",
    )
    parser_create.add_argument("--config", help="Path to the configuration file.")
    parser_create.add_argument(
        "--linelist", help="Path to the input linelist file (.csv)."
    )
    parser_create.add_argument(
        "--in_dir",
        help="Path to the input directory (for storing any resources needed by the report, e.g., shapefiles).",
    )
    parser_create.add_argument("--out_dir", help="Path to the output directory.")
    parser_create.add_argument(
        "--populate_vars",
        action="store_true",
        help="Populate variables in the HTML file before generating the report.",
    )
    ...
    parser_populate = subparsers.add_parser(
        "populate",
        help="Populate any variable placeholders (enclosed by double curly braces) in an HTML file with corresponding values extracted from a linelist file. Any variables that cannot be computed from the linelist will be left as is.",
    )
    ...
    parser_populate.add_argument(
        "--in_file",
        help="Path to the input html file, where the variables will be populated.",
    )
    ...
    parser_list = subparsers.add_parser(
        "list", help="List all variables that can be computed."
    )
    parser_list.add_argument(
        "--json",
        action="store_true",
        help="Output the list of variables in JSON format, otherwise a comma-separated list in plain text.",
    )
