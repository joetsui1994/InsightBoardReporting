...
def preprocess_multi_province_time_series_data(data, parameters):
    ...
    if aggregate_by_epiweek:
        ...
        plot_data["year"], plot_data["epiweek"], _ = (
            plot_data[time_col].dt.isocalendar().values.T
        )
        plot_data = (
            plot_data.groupby(["year", "epiweek", "province"])
            .size()
        ...
        plot_data = (
            plot_data.groupby(["date", "province"]).size().reset_index(name="count")
        )
    ...
    all_dates = pd.date_range(
        plot_data["date"].min(),
        plot_data["date"].max(),
        freq="D" if not aggregate_by_epiweek else "W",
    )
    ...
    plot_data["moving_average"] = (
        plot_data["count"].rolling(window=effective_window).mean()
    )
