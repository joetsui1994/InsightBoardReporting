...
def preprocess(data, config):
    ...
    age_groups = config.get("age_groups", [0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60])
...
def plot(plot_data, config, out_dir):
    ...
    for group in groups:
        ...
        fig.add_trace(
            go.Bar(
                y=group_data["age_group_labels"].astype(str),  # age groups as strings for proper labeling
                x=group_data["male"],
                name="Male",
                orientation="h",
        ...
        num_divs = 4
        max_population = max(group_data["male"].abs().max(), group_data["female"].abs().max())
